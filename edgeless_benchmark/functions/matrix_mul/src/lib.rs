...
impl EdgeFunction for MatrixMulFunction {
    ...
    // example of payload:
    // seed=42,inter_arrival=2000,is_first=true,is_last=false,matrix_size=1000,outputs=0:2:19
    fn handle_init(payload: Option<&[u8]>, _serialized_state: Option<&[u8]>) {
        ...
        for n in output_tokens.into_iter().filter_map(|x| x.parse::<usize>().ok()).collect::<Vec<usize>>() {
