    ...
    fn handle_cast(_src: InstanceId, encoded_message: &[u8]) {
        ...
        if tokens.len() == 5 {
                ...
                (parse(tokens[1]), parse(tokens[2]), parse(tokens[3]), parse(tokens[4]))
            {
                ...
                // Calculate zoom level and adjust max iterations accordingly
                let view_width = bottom_right_x - top_left_x;
...

                {
                    ...
                    bands.into_iter().for_each(|(i, band)| {
                        ...
                        render(band, bytes_per_pixel, band_bounds, band_upper_left, band_lower_right, &colors, max_iterations);
                    });
                }
                ...
                .is_ok()
                {
                    // Generate hash of PNG data after writing
                    let mut hasher = DefaultHasher::new();
                    png_data.get_ref().hash(&mut hasher);
                    let png_hash = hasher.finish();
                    log::info!("calculator: PNG hash for segment {}: {:x}, size: {} bytes", 
                              index, png_hash, png_data.get_ref().len());
...
fn render(
    ...
    for row in 0..bounds.1 {
        for column in 0..bounds.0 {
            ...
            let value: usize = match escape_time(point, max_iterations) {
                None => 0,        // Point is in the Mandelbrot set (black)
                Some(count) => {
                    // Map count to 0-254 range (leaving 0 for set members)
                    let normalized = ((count as f64 / max_iterations as f64) * 254.0) as usize;
                    (254 - normalized).min(254).max(1)
                }
            };
