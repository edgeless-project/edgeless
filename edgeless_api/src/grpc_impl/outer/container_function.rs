...
impl ContainerFunctionAPIClient {
    pub async fn new(api_addr: &str, timeout: std::time::Duration) -> anyhow::Result<Self> {
        Ok(Self {
            guest_api_function:
                match crate::grpc_impl::inner::guest_api_function::GuestAPIFunctionClient::new(
                    api_addr, timeout,
                )
                .await
                {
...
impl GuestAPIFunctionServer {
    pub fn run(
        container_function_api: Box<
            dyn crate::outer::container_function::ContainerFunctionAPI + Send,
        >,
        ...
        let workflow_api = crate::grpc_impl::inner::guest_api_function::GuestAPIFunctionService {
            guest_api_function: tokio::sync::Mutex::new(
                container_function_api.guest_api_function(),
            ),
        };
        Box::pin(async move {
            ...
            if let Ok((_proto, host, port)) = crate::util::parse_http_host(&container_function_url)
            {
                if let Ok(host) = format!("{}:{}", host, port).parse() {
                    log::info!(
                        "Start ContainerFunctionAPI GRPC Server at {}",
                        container_function_url
                    );
