...
pub struct GuestAPIHostClient {
    client:
        crate::grpc_impl::api::guest_api_host_client::GuestApiHostClient<tonic::transport::Channel>,
}
...
impl GuestAPIHostClient {
    pub async fn new(server_addr: &str, retry_interval: Option<u64>) -> anyhow::Result<Self> {
        loop {
            match crate::grpc_impl::api::guest_api_host_client::GuestApiHostClient::connect(
...
impl crate::guest_api_host::GuestAPIHost for GuestAPIHostClient {
    async fn cast(&mut self, event: crate::guest_api_host::OutputEventData) -> anyhow::Result<()> {
        match self
            .client
            .cast(tonic::Request::new(serialize_output_event_data(&event)))
            .await
        {
            Ok(_) => Ok(()),
    ...
    async fn cast_raw(
        &mut self,
        event: crate::guest_api_host::OutputEventDataRaw,
    ) -> anyhow::Result<()> {
        match self
            .client
            .cast_raw(tonic::Request::new(serialize_output_event_data_raw(&event)))
            .await
        {
            Ok(_) => Ok(()),
            Err(err) => Err(anyhow::anyhow!(
                "Communication error while raw-casting an event: {}",
                err.to_string()
            )),
        }
    }
    async fn call(
        &mut self,
        event: crate::guest_api_host::OutputEventData,
    ) -> anyhow::Result<crate::guest_api_function::CallReturn> {
        match self
            .client
            .call(tonic::Request::new(serialize_output_event_data(&event)))
            .await
        {
            Ok(msg) => super::guest_api_function::parse_call_return(&msg.into_inner()),
            Err(err) => Err(anyhow::anyhow!(
                "Communication error while calling a function: {}",
                err.to_string()
            )),
        }
    }
    async fn call_raw(
        ...
        match self
            .client
            .call_raw(tonic::Request::new(serialize_output_event_data_raw(&event)))
            .await
        {
            Ok(msg) => super::guest_api_function::parse_call_return(&msg.into_inner()),
            Err(err) => Err(anyhow::anyhow!(
                "Communication error while raw-calling a function: {}",
                err.to_string()
            )),
        }
    }
    async fn telemetry_log(
        &mut self,
        event: crate::guest_api_host::TelemetryLogEvent,
    ) -> anyhow::Result<()> {
        match self
            .client
    ...
    async fn slf(&mut self) -> anyhow::Result<edgeless_api_core::instance_id::InstanceId> {
        match self.client.slf(tonic::Request::new(())).await {
            Ok(msg) => {
                crate::grpc_impl::common::CommonConverters::parse_instance_id(&msg.into_inner())
            }
            Err(err) => Err(anyhow::anyhow!(
                "Communication error while casting an event: {}",
                err.to_string()
    ...
    async fn delayed_cast(
        &mut self,
        event: crate::guest_api_host::DelayedEventData,
    ) -> anyhow::Result<()> {
        match self
            .client
            .delayed_cast(tonic::Request::new(serialize_delayed_event_data(&event)))
            .await
        {
            Ok(_) => Ok(()),
            Err(err) => Err(anyhow::anyhow!(
                "Communication error while casting a delayed event: {}",
                err.to_string()
            )),
        }
    }
    async fn sync(&mut self, event: crate::guest_api_host::SyncData) -> anyhow::Result<()> {
        match self
            .client
            .sync(tonic::Request::new(serialize_sync_data(&event)))
            .await
        {
            Ok(_) => Ok(()),
            Err(err) => Err(anyhow::anyhow!(
                "Communication error while synchronizing data: {}",
                err.to_string()
            )),
        }
    }
...
impl crate::grpc_impl::api::guest_api_host_server::GuestApiHost for GuestAPIHostService {
    ...
    async fn cast_raw(
        ...
        match self
            ...
            .lock()
            .await
            .cast_raw(parsed_request)
            .await
        {
            Ok(_) => Ok(tonic::Response::new(())),
            Err(err) => Err(tonic::Status::internal(format!(
                "Error when raw-casting an event: {}",
                err
            ))),
        }
    }
    ...
    async fn call(
        ...
        match self.guest_api_host.lock().await.call(parsed_request).await {
            Ok(msg) => Ok(tonic::Response::new(
                super::guest_api_function::serialize_call_return(&msg),
            )),
            Err(err) => Err(tonic::Status::internal(format!(
    ...
    async fn call_raw(
            ...
            .await
        {
            ...
            Err(err) => Err(tonic::Status::internal(format!(
                "Error when raw-calling a function: {}",
                err
            ))),
        }
    }
    ...
    async fn telemetry_log(
        &self,
        event: tonic::Request<crate::grpc_impl::api::TelemetryLogEvent>,
    ) -> Result<tonic::Response<()>, tonic::Status> {
        ...
        match self
            ...
            .lock()
            .await
            .telemetry_log(parsed_request)
            .await
        {
            Ok(_) => Ok(tonic::Response::new(())),
            Err(err) => Err(tonic::Status::internal(format!(
                "Error when emitting a telemetry log event: {}",
                err
            ))),
        }
    }
    ...
    async fn slf(
        ...
        match self.guest_api_host.lock().await.slf().await {
            Ok(msg) => Ok(tonic::Response::new(
                crate::grpc_impl::common::CommonConverters::serialize_instance_id(&msg),
            )),
            Err(err) => Err(tonic::Status::internal(format!(
                "Error when raw-casting an event: {}",
                err
    ...
    async fn delayed_cast(
        &self,
        event: tonic::Request<crate::grpc_impl::api::DelayedEventData>,
    ) -> Result<tonic::Response<()>, tonic::Status> {
        ...
        match self
            ...
            .lock()
            .await
            .delayed_cast(parsed_request)
            .await
        {
            Ok(_) => Ok(tonic::Response::new(())),
            Err(err) => Err(tonic::Status::internal(format!(
                "Error when casting a delayed event: {}",
                err
            ))),
        }
    }
    ...
    async fn sync(
        &self,
        event: tonic::Request<crate::grpc_impl::api::SyncData>,
    ) -> Result<tonic::Response<()>, tonic::Status> {
        let parsed_request = match parse_sync_data(&event.into_inner()) {
            Ok(parsed_request) => parsed_request,
            Err(err) => {
                return Err(tonic::Status::invalid_argument(format!(
                    "Error when parsing a SyncData message: {}",
                    err
                )));
            }
        };
...
fn parse_output_event_data(
    api_instance: &crate::grpc_impl::api::OutputEventData,
) -> anyhow::Result<crate::guest_api_host::OutputEventData> {
        ...
        Some(instance_id) => {
            match crate::grpc_impl::common::CommonConverters::parse_instance_id(instance_id) {
                Ok(dst) => Ok(crate::guest_api_host::OutputEventDataRaw {
                    originator: match &api_instance.originator {
                        Some(instance_id) => {
                            match crate::grpc_impl::common::CommonConverters::parse_instance_id(
                                instance_id,
                            ) {
                                Ok(originator) => originator,
                                Err(err) => {
                                    return Err(anyhow::anyhow!(
                                        "invalid originator field: {}",
                                        err
                                    ))
                                }
                            }
...
fn parse_telemetry_log_event(
    ...
    Ok(crate::guest_api_host::TelemetryLogEvent {
        ...
        log_level: match api_instance.log_level {
            x if x == crate::grpc_impl::api::TelemetryLogLevel::LogError as i32 => {
                crate::guest_api_host::TelemetryLogLevel::Error
            }
            x if x == crate::grpc_impl::api::TelemetryLogLevel::LogWarn as i32 => {
                crate::guest_api_host::TelemetryLogLevel::Warn
            }
            x if x == crate::grpc_impl::api::TelemetryLogLevel::LogInfo as i32 => {
                crate::guest_api_host::TelemetryLogLevel::Info
            }
            x if x == crate::grpc_impl::api::TelemetryLogLevel::LogDebug as i32 => {
                crate::guest_api_host::TelemetryLogLevel::Debug
            }
            x if x == crate::grpc_impl::api::TelemetryLogLevel::LogTrace as i32 => {
                crate::guest_api_host::TelemetryLogLevel::Trace
            }
            x => {
                return Err(anyhow::anyhow!("invalid telemetry event log level: {}", {
                    x
                }))
            }
        },
...
fn parse_delayed_event_data(
    api_instance: &crate::grpc_impl::api::DelayedEventData,
) -> anyhow::Result<crate::guest_api_host::DelayedEventData> {
...
fn parse_sync_data(
    api_instance: &crate::grpc_impl::api::SyncData,
) -> anyhow::Result<crate::guest_api_host::SyncData> {
...
fn serialize_output_event_data(
    event: &crate::guest_api_host::OutputEventData,
) -> crate::grpc_impl::api::OutputEventData {
    crate::grpc_impl::api::OutputEventData {
        originator: Some(
            crate::grpc_impl::common::CommonConverters::serialize_instance_id(&event.originator),
        ),
...
fn serialize_output_event_data_raw(
    event: &crate::guest_api_host::OutputEventDataRaw,
...
fn serialize_telemetry_log_event(
    event: &crate::guest_api_host::TelemetryLogEvent,
    ...
    crate::grpc_impl::api::TelemetryLogEvent {
        ...
        log_level: match event.log_level {
            crate::guest_api_host::TelemetryLogLevel::Error => {
                crate::grpc_impl::api::TelemetryLogLevel::LogError as i32
            }
            crate::guest_api_host::TelemetryLogLevel::Warn => {
                crate::grpc_impl::api::TelemetryLogLevel::LogWarn as i32
            }
            crate::guest_api_host::TelemetryLogLevel::Info => {
                crate::grpc_impl::api::TelemetryLogLevel::LogInfo as i32
            }
            crate::guest_api_host::TelemetryLogLevel::Debug => {
                crate::grpc_impl::api::TelemetryLogLevel::LogDebug as i32
            }
            crate::guest_api_host::TelemetryLogLevel::Trace => {
                crate::grpc_impl::api::TelemetryLogLevel::LogTrace as i32
            }
        },
        msg: event.msg.clone(),
...
fn serialize_delayed_event_data(
    ...
    crate::grpc_impl::api::DelayedEventData {
        ...
        alias: event.alias.clone(),
        msg: event.msg.clone(),
        metadata: Some(crate::grpc_impl::api::EventSerializedMetadata::from(
            &event.metadata,
        )),
    }
}
    ...
    fn serialize_deserialize_delayed_event_data() {
        let messages = vec![
            DelayedEventData {
                ...
                alias: "".to_string(),
                msg: vec![],
                metadata: edgeless_api_core::event_metadata::EventMetadata::from_uints(
                    0x42a42bdecaf00007u128,
                    0x42a42bdecaf00008u64,
                ),
            },
            DelayedEventData {
                originator: edgeless_api_core::instance_id::InstanceId::new(uuid::Uuid::new_v4()),
                ...
                msg: vec![0, 42, 0, 42, 99],
                metadata: edgeless_api_core::event_metadata::EventMetadata::from_uints(
                    0x42a42bdecaf00009u128,
                    0x42a42bdecaf0000au64,
                ),
            },
        ];
        for msg in messages {
