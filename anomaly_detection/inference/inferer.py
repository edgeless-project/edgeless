...
def loop_function(redis_client):
    ...
    name_experiment = "mapreduce_flat"
    folder_name = datetime.now().strftime("%m%d_%H%M_")+name_experiment
    os.makedirs(folder_name, exist_ok=True)
    
    # save dataframes to csv
    file_path_node = folder_name+"/output_node"
    file_path_func = folder_name+"/output_func"
    write_header = not os.path.exists(file_path_node+'.csv')
    df_node_health.to_csv(file_path_node+'.csv', mode='a', header=write_header, index=False)
    df_function_performance.to_csv(file_path_func+'.csv', mode='a', header=write_header, index=False)
