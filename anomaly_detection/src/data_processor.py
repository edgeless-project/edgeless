...
class DataProcessor:    
    ...
    def _build_filename(self, df_type: str, ext: str) -> Path:
        """
        Construct a valid filename for a DataFrame export

        Priority rules:
          1. If EDGELESS_AD_DF_FULLNAME is set    → "<EDGELESS_AD_DF_FULLNAME>_<df_type>.<ext>"
          2. Else if EDGELESS_AD_DF_PREFIX is set → "<EDGELESS_AD_DF_PREFIX>_<timestamp>_<df_type>.<ext>"
          3. Else → "<timestamp>_<df_type>.<ext>"

        Args:
            df_type (str): DataFrame type (e.g., "node_health_data", "performance_data").
            ext (str): File extension (e.g., "csv" or "parquet").

        Returns:
            Path: Full path to the output file, inside EDGELESS_AD_DF_DIR.
        """

        if self.config.EDGELESS_AD_DF_FULLNAME:
            filename = f"{self.config.EDGELESS_AD_DF_FULLNAME}_{df_type}.{ext}"
        elif self.config.EDGELESS_AD_DF_PREFIX:
            filename = f"{self.config.EDGELESS_AD_DF_PREFIX}_{self.timestamp}_{df_type}.{ext}"
        else:
            filename = f"{self.timestamp}_{df_type}.{ext}"

        return self.config.EDGELESS_AD_DF_DIR / filename
    ...
    def save_to_csv(self, df: pd.DataFrame, df_type: str):
        """
        ...
        try:            
            if not df.empty:
                csv_file = self._build_filename(df_type, "csv")

                df.to_csv(csv_file, index=False, header=self.config.EDGELESS_AD_DF_INCLUDE_COLUMNS)
    ...
    def save_to_parquet(self, df: pd.DataFrame, df_type: str):
        """
        Save DataFrames to CSV files if enabled.
        ...
        try:
            if not df.empty:
                parquet_file = self._build_filename(df_type, "parquet")
    ...
    def load_physical_logical_mapping_from_file(self, file_path: Path) -> pd.DataFrame:
        """
        Load mapping between physical execution and logical execution from file.
        
        Args:
            file_path (Path): Path to the mapping file
            
        Returns:
            pd.DataFrame: DataFrame with columns: physical_uuid, node_uuid, logical_uuid, workflow_uuid
        """
        try:
            ...
            if not file_path.exists():
                self.logger.error(f"Mapping file does not exist: {file_path}")
                return pd.DataFrame()
            ...
            if file_path_str.endswith('.csv'):
                ...
                self.logger.error(f"Unsupported file format: {file_path_str}")
                return pd.DataFrame()
            
            # Validate required columns
            required_columns = ['physical_id', 'node_id', 'logical_id', 'workflow_id']
            missing_columns = [col for col in required_columns if col not in df.columns]
            if missing_columns:
                self.logger.error(f"Missing required columns in mapping file: {missing_columns}")
                return pd.DataFrame()
            ...
            df = df.rename(columns={
                ...
                'node_id': 'node_uuid', 
                'logical_id': 'logical_uuid',
                'workflow_id': 'workflow_uuid'
            })
            
            # Filter out rows with empty physical_id or node_id
            df = df.dropna(subset=['physical_uuid', 'node_uuid'])
            
            # Add missing class_id column if not present (use logical_uuid as fallback)
            if 'class_id' not in df.columns:
                df['class_id'] = df['logical_uuid']
            
            self.logger.debug(f"Loaded mapping file: {file_path} ({len(df)} records)")
            return df
            
        except Exception as e:
            self.logger.error(f"Error loading mapping file {file_path}: {str(e)}")
            return pd.DataFrame()