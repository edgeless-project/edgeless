...
pub struct EdgelessBalSettings {
    ...
    pub nodes: Vec<edgeless_dataplane::core::EdgelessDataplanePeerSettings>,
}
...
pub async fn edgeless_bal_main(settings: EdgelessBalSettings) {
    ...
    let data_plane =
        edgeless_dataplane::handle::DataplaneProvider::new(settings.balancer_id.clone(), settings.invocation_url.clone(), settings.nodes.clone())
            .await;
