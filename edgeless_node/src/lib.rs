...
pub struct EdgelessNodeSettingsPeer {
    id: uuid::Uuid,
...
pub struct EdgelessNodeSettings {
    ...
    pub peers: Vec<EdgelessNodeSettingsPeer>,
}
...
pub async fn edgeless_node_main(settings: EdgelessNodeSettings) {
    ...
    let data_plane = data_plane::DataPlaneChainProvider::new(settings.node_id.clone(), settings.invocation_url.clone(), settings.peers.clone()).await;
