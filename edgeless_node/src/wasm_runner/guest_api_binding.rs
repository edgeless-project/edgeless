...
pub async fn telemetry_log(
    ...
    let target = super::helpers::load_string_from_vm(
    ...
    let msg =
        super::helpers::load_string_from_vm(&mut caller.as_context_mut(), &mem, msg_ptr, msg_len)?;
...
pub async fn cast_raw(
    ...
    let instance_id = edgeless_api::function_instance::InstanceId {
        node_id: uuid::Uuid::from_bytes(
            node_id
                .try_into()
                .map_err(|_| wasmtime::Error::msg("uuid error"))?,
        ),
    ...
    let payload = super::helpers::load_string_from_vm(
        &mut caller.as_context_mut(),
        &mem,
        payload_ptr,
        payload_len,
...
pub async fn call_raw(
    ...
    match call_ret {
        ...
        edgeless_dataplane::core::CallRet::Reply(data) => {
            ...
            let data_ptr = super::helpers::copy_to_vm(
                &mut caller.as_context_mut(),
                &mem,
                &alloc,
                data.as_bytes(),
...
pub async fn sync(
    ...
    let mem = get_memory(&mut caller)?;
    let state = super::helpers::load_string_from_vm(
        &mut caller.as_context_mut(),
        &mem,
        state_ptr,
        state_len,
...
pub(crate) fn get_memory(
    caller: &mut wasmtime::Caller<'_, super::guest_api_binding::GuestAPI>,
...
pub(crate) fn get_alloc(
    caller: &mut wasmtime::Caller<'_, super::guest_api_binding::GuestAPI>,
) -> wasmtime::Result<wasmtime::TypedFunc<i32, i32>> {
    caller
