...
impl Orchestrator {
    pub async fn new(
        ...
        let _ = subscriber_sender
            .send(
                crate::domain_subscriber::DomainSubscriberRequest::RegisterOrcSender(
                    sender.clone(),
                ),
            )
            .await;
    ...
    pub fn get_api_client(&mut self) -> Box<dyn edgeless_api::outer::orc::OrchestratorAPI + Send> {
        Box::new(OrchestratorClient {
            function_instance_client: Box::new(OrchestratorFunctionInstanceOrcClient {
    ...
    > for OrchestratorFunctionInstanceOrcClient
{
    ...
    > {
        ...
        if let Err(err) = self
            .sender
            .send(OrchestratorRequest::StartFunction(request, reply_sender))
            .await
        {
    ...
    async fn stop(
        ...
        match self
            .sender
            .send(OrchestratorRequest::StopFunction(id))
            .await
        {
            Ok(_) => Ok(()),
            Err(err) => Err(anyhow::anyhow!(
    ...
    > for ResourceConfigurationClient
{
    ...
    > {
        ...
        if let Err(err) = self
            .sender
            .send(OrchestratorRequest::StartResource(request, reply_sender))
            .await
        {
            return Err(anyhow::anyhow!(
    ...
    async fn stop(
        ...
        match self
            .sender
            .send(OrchestratorRequest::StopResource(id))
            .await
        {
            Ok(_) => Ok(()),
            Err(err) => Err(anyhow::anyhow!(
