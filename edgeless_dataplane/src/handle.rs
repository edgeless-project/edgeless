...
impl DataplaneHandle {
    ...
    /// Send a `cast` event.
    pub async fn send(&mut self, target: InstanceId, msg: String) {
        self.send_inner(target, Message::Cast(msg), timestamp_utc(), 0).await;
    }

    // Send a `call` event and wait for the return event. Internally, this sets
    // up a receiver override to handle the message before it would be sent to
    // the `receive_next` function.
    pub async fn call(&mut self, target: InstanceId, msg: String) -> CallRet {
        let (sender, receiver) = futures::channel::oneshot::channel::<(InstanceId, Message)>();
    ...
    pub async fn reply(&mut self, target: InstanceId, channel_id: u64, msg: CallRet) {
        self.send_inner(
            target,
            match msg {
    ...
    // passes the Message through the output_chain until it finds a link which
    // can process it fully (FINAL)
    async fn send_inner(&mut self, target: InstanceId, msg: Message, created: function_instance::EventTimestamp, channel_id: u64) {
