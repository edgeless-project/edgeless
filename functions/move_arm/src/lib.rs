...
impl EdgeFunction for MoveArmFun {
    fn handle_cast(_src: InstanceId, encoded_message: &[u8]) {
        ...
        if tokens.len() == 3 && tokens[0] == "checktemp" {
            ...
            json_params.push_str(&format!(
                r#"{{"pubid": "dda_move_arm", "pattern": "action", "params": "{}"}}"#,
                mov_para.to_string()
            ));
    ...
    fn handle_call(_src: InstanceId, encoded_message: &[u8]) -> CallRet {
        log::info!(
            "Move Arm Function: handle_call called {:?} - but not implemented!",
            std::str::from_utf8(encoded_message)
        );
        CallRet::NoReply
    }
