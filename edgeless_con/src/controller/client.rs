...
impl edgeless_api::workflow_instance::WorkflowInstanceAPI for ControllerWorkflowInstanceClient {
    async fn start(
        ...
        if let Err(err) = self
            .sender
            .send(super::ControllerRequest::Start(
                request.clone(),
                reply_sender,
            ))
            .await
    ...
    async fn list(&mut self) -> anyhow::Result<Vec<edgeless_api::workflow_instance::WorkflowId>> {
        ...
        if let Err(err) = self
            .sender
            .send(super::ControllerRequest::List(reply_sender))
            .await
        {
            anyhow::bail!("Controller Channel Error: {}", err);
        }
    ...
    async fn inspect(
        ...
        if let Err(err) = self
            .sender
            .send(super::ControllerRequest::Inspect(id.clone(), reply_sender))
            .await
        {
            anyhow::bail!("Controller Channel Error: {}", err);
        }
    ...
    > {
        ...
        if let Err(err) = self
            .sender
            .send(super::ControllerRequest::Domains(
                domain_id.clone(),
                reply_sender,
            ))
            .await
    ...
    async fn migrate(
        ...
        if let Err(err) = self
            .sender
            .send(super::ControllerRequest::Migrate(
                request.clone(),
                reply_sender,
            ))
            .await
