...
pub fn edgeless_inabox_main(
    ...
    for entry in std::fs::read_dir(std::env::current_dir().expect("Cannot read CWD"))? {
        ...
        let filename = String::from(
            path.file_name()
                .expect("Cannot retrieve the name of a file")
                .to_string_lossy(),
        );
    ...
    let orc_conf = if orc_file.exists() {
        Some(toml::from_str::<edgeless_orc::EdgelessOrcSettings>(
            &std::fs::read_to_string(orc_file)?,
        )?)
    } else {
        None
    };
    ...
    let bal_conf = if bal_file.exists() {
        Some(toml::from_str::<edgeless_bal::EdgelessBalSettings>(
            &std::fs::read_to_string(bal_file)?,
        )?)
    } else {
        None
    };
    ...
    let con_conf = if con_file.exists() {
        Some(toml::from_str::<edgeless_con::EdgelessConSettings>(
            &std::fs::read_to_string(con_file)?,
        )?)
    } else {
        None
    };
...
mod tests {
    ...
    fn test_start() -> anyhow::Result<()> {
        ...
        let node_conf = dir
            .join(std::path::Path::new("node.toml"))
            .to_str()
            .unwrap()
            .to_string();
        let orc_conf = dir
            .join(std::path::Path::new("orchestrator.toml"))
            .to_str()
            .unwrap()
            .to_string();
        let bal_conf = dir
            .join(std::path::Path::new("balancer.toml"))
            .to_str()
            .unwrap()
            .to_string();
        let con_conf = dir
            .join(std::path::Path::new("controller.toml"))
            .to_str()
            .unwrap()
            .to_string();
        ...
        edgeless_api::util::create_template(
            orc_conf.as_str(),
            edgeless_orc::edgeless_orc_default_conf().as_str(),
        )?;
        edgeless_api::util::create_template(
            bal_conf.as_str(),
        ...
        edgeless_api::util::create_template(
            con_conf.as_str(),
            edgeless_con::edgeless_con_default_conf().as_str(),
        )?;
