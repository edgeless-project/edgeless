...
enum ContainerFunctionRequest {
    Boot(
        edgeless_api::guest_api_function::BootData,
        tokio::sync::oneshot::Sender<anyhow::Result<()>>,
    ),
    Init(
        edgeless_api::guest_api_function::FunctionInstanceInit,
        tokio::sync::oneshot::Sender<anyhow::Result<()>>,
    ),
...
impl ContainerFunction {
    ...
    async fn main_task(receiver: futures::channel::mpsc::UnboundedReceiver<ContainerFunctionRequest>) {
        ...
        // Always-visible diagnostic (doesn't depend on log backend)
        eprintln!("MAIN_TASK started");
        ...
        while let Some(req) = receiver.next().await {
            match req {
                ContainerFunctionRequest::Boot(boot_data, ack) => {
                    eprintln!(
                        "BOOT received (endpoint={}, instance_id={})",
                        boot_data.guest_api_host_endpoint, boot_data.instance_id
                    );

                    if std::mem::discriminant(&fsm) != std::mem::discriminant(&FiniteStateMachine::PreBoot) {
                        let _ = ack.send(Err(anyhow::anyhow!("boot received while not in PreBoot")));
                        ...
                        .await
                        {
                            Ok(client) => {
                                ...
                                let _ = ack.send(Ok(()));
                                eprintln!("BOOT done -> state Booted");
                            }
                            Err(err) => {
                                log::error!("container function boot error: {}", err);
                                fsm = FiniteStateMachine::Error;

                                let _ = ack.send(Err(anyhow::anyhow!("boot error: {}", err)));
                                eprintln!("BOOT error: {}", err);
                            }
                        }
                    }
                }
                ContainerFunctionRequest::Init(init_data, ack) => {
                    eprintln!(
                        "INIT received (payload_len={}, state_len={})",
                        init_data.init_payload.len(),
                        init_data.serialized_state.len()
                    );

                    if std::mem::discriminant(&fsm) != std::mem::discriminant(&FiniteStateMachine::Booted) {
                        let _ = ack.send(Err(anyhow::anyhow!("init received while not in Booted")));
                        log::error!("received init command while not in a booted state: ignored");
                    } else {
                        // Add init logic here
                        eprintln!("INIT_HOOK hello from inside init");

                        // IMPORTANT: don't log secret values in production; log presence only.
                        match std::env::var("DEMO_SECRET") {
                            Ok(value) => eprintln!("DEMO_SECRET is: {}", value),
                            Err(e) => eprintln!("Couldn't read DEMO_SECRET: {}", e),
                        }

                        fsm = FiniteStateMachine::Initialized;

                        eprintln!("INIT_HOOK done -> state Initialized");
                        let _ = ack.send(Ok(()));
                    }
                }
                ContainerFunctionRequest::Cast(event) => {
                    ...
                    if std::mem::discriminant(&fsm) != std::mem::discriminant(&FiniteStateMachine::Initialized) {
                        ...
                        // Add cast logic here.
                        log::info!("CAST_HOOK hello from inside cast");
...
impl edgeless_api::guest_api_function::GuestAPIFunction for GuestAPIFunctionClient {
    async fn boot(&mut self, boot_data: edgeless_api::guest_api_function::BootData) -> anyhow::Result<()> {
        ...
        self.sender
            .send(ContainerFunctionRequest::Boot(boot_data.clone(), ack_tx))
            .await
            .map_err(|err| anyhow::anyhow!("GuestAPIFunction::boot channel error: {}", err))?;

        ack_rx
            .await
            .map_err(|err| anyhow::anyhow!("GuestAPIFunction::boot ack dropped: {}", err))?
    }
    ...
    async fn init(&mut self, init_data: edgeless_api::guest_api_function::FunctionInstanceInit) -> anyhow::Result<()> {
        ...
        self.sender
            .send(ContainerFunctionRequest::Init(init_data.clone(), ack_tx))
            .await
            .map_err(|err| anyhow::anyhow!("GuestAPIFunction::init channel error: {}", err))?;

        ack_rx
            .await
            .map_err(|err| anyhow::anyhow!("GuestAPIFunction::init ack dropped: {}", err))?
    }
    ...
    async fn cast(&mut self, event: edgeless_api::guest_api_function::InputEventData) -> anyhow::Result<()> {
        match self.sender.send(ContainerFunctionRequest::Cast(event.clone())).await {
            Ok(_) => Ok(()),
            Err(err) => Err(anyhow::anyhow!("GuestAPIFunction::cast channel error: {}", err)),
        }
    }
    ...
    async fn call(
        ...
        let (reply_sender, reply_receiver) =
            tokio::sync::oneshot::channel::<anyhow::Result<edgeless_api::guest_api_function::CallReturn>>();
    ...
    async fn stop(&mut self) -> anyhow::Result<()> {
        match self.sender.send(ContainerFunctionRequest::Stop()).await {
            Ok(_) => Ok(()),
            Err(err) => Err(anyhow::anyhow!("GuestAPIFunction::stop channel error: {}", err)),
        }
    }
}
