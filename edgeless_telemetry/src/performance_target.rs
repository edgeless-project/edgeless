...
#[derive(Default)]
pub struct Metrics {
    pub function_execution_times: std::collections::HashMap<
        edgeless_api::function_instance::ComponentId,
        Vec<edgeless_api::node_registration::Sample>,
    >,
    pub function_transfer_times: std::collections::HashMap<
        edgeless_api::function_instance::ComponentId,
        Vec<edgeless_api::node_registration::Sample>,
    >,
    pub function_log_entries: std::collections::HashMap<
        edgeless_api::function_instance::ComponentId,
        Vec<edgeless_api::node_registration::FunctionLogEntry>,
    >,
}
...
mod tests {
    ...
    fn test_performance_target_get_metrics() {
        ...
        let event_tags =
            std::collections::BTreeMap::from([("FUNCTION_ID".to_string(), fid.to_string())]);
        ...
        for i in 0..10 {
            ...
            target.handle(
                &crate::telemetry_events::TelemetryEvent::FunctionInit(
            ...
            target.handle(
                &crate::telemetry_events::TelemetryEvent::FunctionInvocationCompleted(
                    std::time::Duration::from_secs(*expected_execution.last().unwrap() as u64),
                ),
                &event_tags,
            );
            target.handle(
                &crate::telemetry_events::TelemetryEvent::FunctionTransfer(
                    std::time::Duration::from_secs(*expected_transfer.last().unwrap() as u64),
                ),
                &event_tags,
            );
        ...
        assert_eq!(
            expected_execution,
            samples.iter().map(|x| x.sample).collect::<Vec<f64>>()
        );
